...
SELECT
	...
	T0.poin_kehadiran,
	REPLACE(T0.ket_kehadiran, ' .', '') AS ket_kehadiran,
...
FROM (
	SELECT
		...
		CASE WHEN T0.poin_kehadiran = 7 THEN 'tidak hadir tanpa konfirmasi.'
			...
			WHEN T0.poin_kehadiran BETWEEN 1 AND 3 THEN CONCAT(T0.ket_datang, '. ', T0.ket_pulang, '. ', T0.ket_istirahat)
		...
		CASE WHEN T0.poin_kehadiran = 7 THEN 'tidak hadir tanpa konfirmasi'
			...
			WHEN T0.poin_kehadiran BETWEEN 1 AND 3 THEN 'hadir, tapi tidak lengkap'
	...
	FROM (
		SELECT
			...
			CASE
		   	...
		   	WHEN T0.ket_luar_kota IS NOT NULL AND T0.poin_datang = 1 THEN 5
				...
				ELSE (T0.poin_datang + T0.poin_pulang + T0.poin_istirahat)
		...
		FROM (
			SELECT
				...
				CASE WHEN T1.durasi_telat_datang > 0 THEN 1 WHEN T1.jam_datang IS NULL THEN 0.5 ELSE 0 END AS poin_datang,
				CASE WHEN T1.jam_datang IS NULL THEN 'absen datang tidak ada' ELSE 
				...
				CASE WHEN T1.durasi_duluan_pulang > 0 THEN 1 WHEN T1.jam_pulang IS NULL THEN 0.5 ELSE 0 END AS poin_pulang,
				CASE WHEN T1.jam_pulang IS NULL THEN 'absen pulang tidak ada' ELSE 
				...
				T1.durasi_istirahat,
				CASE WHEN T1.durasi_istirahat > 60 THEN 1 
					...
					ELSE 0 END AS poin_istirahat,
				CASE WHEN T1.jam_mulai_istirahat IS NULL AND T1.jam_selesai_istirahat IS NOT NULL THEN 'absen istirahat tidak lengkap' 
					WHEN T1.jam_mulai_istirahat IS NOT NULL AND T1.jam_selesai_istirahat IS NULL THEN 'absen istirahat tidak lengkap' 
					ELSE (CASE WHEN T1.durasi_istirahat > 60 THEN CONCAT('istirahat lewat ', T1.durasi_istirahat, ' menit') ELSE '' END) END AS ket_istirahat,
			...
			LEFT JOIN (
				SELECT
					...
					T0.jam_datang, 
					CASE WHEN T0.jam_datang > TIME('08:30:00') THEN TIMESTAMPDIFF(MINUTE, TIME('08:30:00'), T0.jam_datang) ELSE 0 END AS durasi_telat_datang,
					T0.jam_pulang,
					CASE WHEN T0.jam_pulang < (CASE WHEN DAYOFWEEK(T0.tanggal) = 7 THEN TIME('16:00:00') ELSE TIME('17:00:00') END) 
						THEN TIMESTAMPDIFF(MINUTE, T0.jam_pulang, (CASE WHEN DAYOFWEEK(T0.tanggal) = 7 THEN TIME('16:00:00') ELSE TIME('17:00:00') END))+1 
				...
				FROM (
					SELECT
						...
						MIN(CASE WHEN T0.attendance_type = 'absen_masuk' AND T0.attendance_break = 0 THEN TIME(T0.attendance_datetime) END) AS jam_datang,
						MAX(CASE WHEN T0.attendance_type = 'absen_keluar' AND T0.attendance_break = 0 THEN TIME(T0.attendance_datetime) END) AS jam_pulang,
